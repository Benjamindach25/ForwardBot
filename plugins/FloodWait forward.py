...
async def send_for_forward(bot, message):
    ...
    buttons = [[
        ...
        InlineKeyboardButton('CLOSE', callback_data=f'forward#close#{chat_id}#{lst_msg_id}')
    ]]
...
async def forward_files(lst_msg_id, chat, msg, bot, user_id):
    ...
    caption_text = CAPTION.get(user_id) if CAPTION.get(user_id) else FILE_CAPTION
    ...
    try:
        async for message in bot.iter_messages(chat, lst_msg_id, CURRENT.get(user_id) if CURRENT.get(user_id) else 0):
            ...
            try:
                caption = caption_text.format(file_name=media.file_name, file_size=get_size(media.file_size), caption=message.caption or '')
                ...
                await msg.reply(f"FloodWait Error! Sleeping for {e.value} seconds...")
                await asyncio.sleep(e.value + 5) # Additional 5 seconds delay
                caption = caption_text.format(file_name=media.file_name, file_size=get_size(media.file_size), caption=message.caption or '')
                await bot.send_cached_media(
                    chat_id=CHANNEL.get(user_id),
                    file_id=media.file_id,
                ...
                await msg.reply(f"FloodWaitError! Sleeping for {e.value} seconds...")
                await asyncio.sleep(e.value + 5) # Additional 5 seconds delay
                continue
            except Exception as e:
                logger.exception(f"Error while forwarding: {e}")
